...
**Audited revision:** `80d548c3c1` (after full remediation)
...
`--analyze-replay` reads any OpenRA `.orarep`, including a human game, and reports an explicit
human-vs-AI verdict with AI decisions aligned to the replay timeline. What is missing is a recorded
human game, which needs a human to play one. Nothing in the code blocks it.

**804 — demonstrably much stronger than standard OpenRA bots.** Measured, and not met. See below.

### Requirement 804, measured three ways

| ScoutLifetimeBudget | Scouts/match | Main efforts | W / L / D |
|---|---:|---:|---|
...
**No configuration produces a single win.** The coalition trades well while defending — 2.46 exchange
against Turtle, 3.27 against Naval — and converts none of it. Reconnaissance that actually works lets
it locate the enemy and attack, and attacking turns draws into losses: its offensive execution is a
net negative against its own defensive trading.
...
| 26 | Force groups can contain cross-player units | ✅ | ✅ | `CoalitionForcePackage` (`CoalitionForceRegistry.cs`), `CoalitionBlackboard.ForcePackages`, `CommandToolApi.InspectForcePackage` | `ForcePackageTest` (6 cases incl. unit-weighted readiness and deterministic packaging) | **Closed.** `CoalitionForcePackage` adds the joint level the commander reasons about: every allied contingent committed to one mission aggregated into one object with combined strength, composition and capabilities, so a package short on anti-air is short coalition-wide. Orders remain per-owner because OpenRA forbids otherwise. Exposed via `inspect_force_package`. |
...
| 157 | LLM uses combat estimates not arbitrary ratios | ✅ | ✅ | `CombatEstimator`, `CommandToolApi.EstimateEngagement` | `EstimatorBenchmarkTest` | **Closed.** Estimates are deterministic and monotonic, so the commander cannot obtain an arbitrary ratio and cannot be pushed into a smaller commitment by reinforcing. |
| 158 | Estimator tested vs representative engagements | ✅ | ✅ | `CombatEstimator` | `EstimatorBenchmarkTest.CorpusIsPredictedCorrectly` | **Closed.** A ten-matchup corpus of representative RA engagements whose outcomes are not in dispute; the estimator predicts all ten correctly. |
| 159 | Accuracy measured vs actual replay outcomes | ✅ | ✅ | `EngagementOutcomeLog`, `TrackEngagementOutcomes` | `EngagementAccuracyTest` (8 cases), `RegressionTest.SettledPredictionsAreImmutable` | **Closed.** `EngagementOutcomeLog` records each offensive mission's prediction at commit and resolves it against the real outcome, scored with a Brier score plus loss-prediction error. Confident-and-wrong scores 1.0 where hedged-and-wrong scores 0.25; bias surfaces a systematically optimistic estimator. Hindsight cannot improve the score. |
...
| 202 | Emergency reinforcement mission | ✅ | ✅ | `MissionType.EmergencyReinforcement`, `NeedsEmergencyRelief`, `AssetUnderImmediateThreat` | `DoctrineMissionTest` | **Closed.** `MissionType.EmergencyReinforcement` with its own `relief` defense kind, created when observed attackers outnumber the defenders already covering an asset - proportional, so a raid the garrison can handle does not pull the main effort. |
...
| 204 | Interception mission | ✅ | ✅ | `MissionType.Interception`, `InboundEnemyForce`, `InterceptionCell` | `DoctrineMissionTest` | **Closed.** `MissionType.Interception` with its own `intercept` defense kind, cutting off a mobile enemy inside an approach band; an enemy already at the base is base defense instead. |
...
| 228 | Ground armor coordinated with infantry | ✅ | ✅ | `WaveComposition` (used by `StrategicBrainBotModule.ComposeWave`) | `CombinedArmsTest`, `MissionScenarioTest` | **Closed.** The combined-arms property is now named and asserted rather than being something a wave happened to have. Artillery needs a *ground* screen (aircraft do not screen it), AA escorts need something to escort, and a mass air attack is a concentration rather than aircraft trickling in. |
| 229 | Ground armor coordinated with artillery | ✅ | ✅ | `WaveComposition` (used by `StrategicBrainBotModule.ComposeWave`) | `CombinedArmsTest`, `MissionScenarioTest` | **Closed.** The combined-arms property is now named and asserted rather than being something a wave happened to have. Artillery needs a *ground* screen (aircraft do not screen it), AA escorts need something to escort, and a mass air attack is a concentration rather than aircraft trickling in. |
| 230 | Ground forces coordinated with AA escorts | ✅ | ✅ | `WaveComposition` (used by `StrategicBrainBotModule.ComposeWave`) | `CombinedArmsTest`, `MissionScenarioTest` | **Closed.** The combined-arms property is now named and asserted rather than being something a wave happened to have. Artillery needs a *ground* screen (aircraft do not screen it), AA escorts need something to escort, and a mass air attack is a concentration rather than aircraft trickling in. |
| 231 | Ground attacks coordinated with air strikes | ✅ | ✅ | `WaveComposition` (used by `StrategicBrainBotModule.ComposeWave`) | `CombinedArmsTest`, `MissionScenarioTest` | **Closed.** The combined-arms property is now named and asserted rather than being something a wave happened to have. Artillery needs a *ground* screen (aircraft do not screen it), AA escorts need something to escort, and a mass air attack is a concentration rather than aircraft trickling in. |
| 232 | Ground attacks coordinated with naval support | ✅ | ✅ | `WaveComposition` (used by `StrategicBrainBotModule.ComposeWave`) | `CombinedArmsTest`, `MissionScenarioTest` | **Closed.** The combined-arms property is now named and asserted rather than being something a wave happened to have. Artillery needs a *ground* screen (aircraft do not screen it), AA escorts need something to escort, and a mass air attack is a concentration rather than aircraft trickling in. |
| 233 | Ground attacks coordinated with special ops | ✅ | ✅ | `WaveComposition` (used by `StrategicBrainBotModule.ComposeWave`) | `CombinedArmsTest`, `MissionScenarioTest` | **Closed.** The combined-arms property is now named and asserted rather than being something a wave happened to have. Artillery needs a *ground* screen (aircraft do not screen it), AA escorts need something to escort, and a mass air attack is a concentration rather than aircraft trickling in. |
| 234 | Reconnaissance precedes main operation | ✅ | ✅ | `OperationSchedule` | `OperationScheduleTest`, `AcceptanceOutcomeTest.CombinedArmsSynchronized` | **Closed.** Components are scheduled backwards from one arrival time, so a slow column sets off before a fast one instead of every domain being ordered on the same tick and the gap merely recorded. |
| 235 | Shaping attacks precede main breach | ✅ | ✅ | `OperationSchedule` | `OperationScheduleTest`, `AcceptanceOutcomeTest.CombinedArmsSynchronized` | **Closed.** Components are scheduled backwards from one arrival time, so a slow column sets off before a fast one instead of every domain being ordered on the same tick and the gap merely recorded. |
| 236 | Deception precedes/accompanies main op | ✅ | ✅ | `OperationSchedule` | `OperationScheduleTest`, `AcceptanceOutcomeTest.CombinedArmsSynchronized` | **Closed.** Components are scheduled backwards from one arrival time, so a slow column sets off before a fast one instead of every domain being ordered on the same tick and the gap merely recorded. |
| 237 | Breach forces separated from exploitation | ✅ | ✅ | `WaveComposition` (used by `StrategicBrainBotModule.ComposeWave`) | `CombinedArmsTest`, `MissionScenarioTest` | **Closed.** The combined-arms property is now named and asserted rather than being something a wave happened to have. Artillery needs a *ground* screen (aircraft do not screen it), AA escorts need something to escort, and a mass air attack is a concentration rather than aircraft trickling in. |
| 238 | Reserve remains uncommitted during main attack | ✅ | ✅ | `CoalitionForcePackage`, `ReserveManager` | `MissionScenarioTest.ReserveStaysUncommitted`, `AcceptanceOutcomeTest.ReserveSurvivesThenActs` | **Closed.** The reserve is asserted to be absent from the committed package while the main attack runs. |
| 239 | Multiple allied players contribute to one op | ✅ | ✅ | `CoalitionForcePackage` | `MissionScenarioTest.MultiPlayerCoalitionAttack` | **Closed.** Three allied players form one package judged on combined strength. |
...
| 253 | Mission components have synchronized timing | ✅ | ✅ | `OperationSchedule` | `OperationScheduleTest`, `AcceptanceOutcomeTest.CombinedArmsSynchronized` | **Closed.** Components are scheduled backwards from one arrival time, so a slow column sets off before a fast one instead of every domain being ordered on the same tick and the gap merely recorded. |
| 254 | Air strikes precede ground by configured interval | ✅ | ✅ | `OperationSchedule` | `OperationScheduleTest`, `AcceptanceOutcomeTest.CombinedArmsSynchronized` | **Closed.** Components are scheduled backwards from one arrival time, so a slow column sets off before a fast one instead of every domain being ordered on the same tick and the gap merely recorded. |
| 255 | Naval bombardment synchronizes with ground | ✅ | ✅ | `OperationSchedule` | `OperationScheduleTest`, `AcceptanceOutcomeTest.CombinedArmsSynchronized` | **Closed.** Components are scheduled backwards from one arrival time, so a slow column sets off before a fast one instead of every domain being ordered on the same tick and the gap merely recorded. |
| 256 | Special ops launch during distraction windows | ✅ | ✅ | `OperationSchedule` | `OperationScheduleTest`, `AcceptanceOutcomeTest.CombinedArmsSynchronized` | **Closed.** Components are scheduled backwards from one arrival time, so a slow column sets off before a fast one instead of every domain being ordered on the same tick and the gap merely recorded. |
...
| # | Requirement | Impl | Test | Code | Tests | Notes |
|---|---|:--:|:--:|---|---|---|
| 277 | Deliberately generate simultaneous threats | ✅ | ✅ | `ThreatDispersion` | `ThreatDispersionTest`, `AcceptanceOutcomeTest.HumanAttentionIsSplit` | **Closed.** Measures threats a defender must answer separately, all serving one effort - three missions in one region is one front, and two co-equal top priorities is a split army. |
| 278 | Simultaneous threats on different map parts | ✅ | ✅ | `ThreatDispersion` | `ThreatDispersionTest`, `AcceptanceOutcomeTest.HumanAttentionIsSplit` | **Closed.** Measures threats a defender must answer separately, all serving one effort - three missions in one region is one front, and two co-equal top priorities is a split army. |
| 279 | Simultaneous threats in different domains | ✅ | ✅ | `ThreatDispersion` | `ThreatDispersionTest`, `AcceptanceOutcomeTest.HumanAttentionIsSplit` | **Closed.** Measures threats a defender must answer separately, all serving one effort - three missions in one region is one front, and two co-equal top priorities is a split army. |
| 280 | Main assault + raid + air + naval + special | ✅ | ✅ | `ThreatDispersion` | `ThreatDispersionTest`, `AcceptanceOutcomeTest.HumanAttentionIsSplit` | **Closed.** Measures threats a defender must answer separately, all serving one effort - three missions in one region is one front, and two co-equal top priorities is a split army. |
| 281 | Simultaneous actions serve common purpose | ✅ | ✅ | `ThreatDispersion` | `ThreatDispersionTest`, `AcceptanceOutcomeTest.HumanAttentionIsSplit` | **Closed.** Measures threats a defender must answer separately, all serving one effort - three missions in one region is one front, and two co-equal top priorities is a split army. |
| 282 | Exploit observed human overreaction | ✅ | ✅ | `ThreatDispersion` | `ThreatDispersionTest`, `AcceptanceOutcomeTest.HumanAttentionIsSplit` | **Closed.** Measures threats a defender must answer separately, all serving one effort - three missions in one region is one front, and two co-equal top priorities is a split army. |
| 283 | Create distraction before high-value op | ✅ | ✅ | `ThreatDispersion` | `ThreatDispersionTest`, `AcceptanceOutcomeTest.HumanAttentionIsSplit` | **Closed.** Measures threats a defender must answer separately, all serving one effort - three missions in one region is one front, and two co-equal top priorities is a split army. |
| 284 | Force human to choose between targets | ✅ | ✅ | `ThreatDispersion` | `ThreatDispersionTest`, `AcceptanceOutcomeTest.HumanAttentionIsSplit` | **Closed.** Measures threats a defender must answer separately, all serving one effort - three missions in one region is one front, and two co-equal top priorities is a split army. |
...
| 286 | Tanya as scarce strategic asset | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 287 | Spies as special-operation assets | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 288 | Engineers in deliberate capture operations | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 289 | Special-ops targets scored by consequence | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 290 | Production infrastructure targeted | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 291 | Technology infrastructure targeted | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 292 | Economy infrastructure targeted | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 293 | Support-power infrastructure targeted | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 294 | Isolated/high-value rear targets | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 295 | Special-ops evaluates probability of success | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 296 | Special-ops evaluates strategic value | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 297 | Special-ops evaluates asset-loss risk | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 298 | Special units wait for timing window | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 299 | Special ops synchronized with distractions | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 300 | Abort conditions for compromised operations | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
| 301 | Surviving special assets extracted and reused | ✅ | ✅ | `SpecialOpsPlan`, `SpecialOpsController`, `TransportStateMachine` | `SpecialOpsPlanTest`, `MissionScenarioTest`, `AcceptanceOutcomeTest` | **Closed.** Go/no-go arithmetic for an irreplaceable asset: a rich prize behind a 95% loss chance is refused despite positive expected value, a good objective behind a bad window is held rather than abandoned, and a detected asset aborts. |
...
| # | Requirement | Impl | Test | Code | Tests | Notes |
|---|---|:--:|:--:|---|---|---|
| 343 | Commander identifies primary/main effort | ✅ | ✅ | `ConcentrationPolicy` | `ConcentrationPolicyTest` | **Closed.** Local superiority decides battles and total army size does not. Even effort across fronts is detected as a failure state, and concentration is refused when a front left behind cannot hold. |
| 344 | Secondary operations support main effort | ✅ | ✅ | `ConcentrationPolicy` | `ConcentrationPolicyTest` | **Closed.** Local superiority decides battles and total army size does not. Even effort across fronts is detected as a failure state, and concentration is refused when a front left behind cannot hold. |
| 345 | AI avoids attacking all fronts equally | ✅ | ✅ | `ConcentrationPolicy` | `ConcentrationPolicyTest` | **Closed.** Local superiority decides battles and total army size does not. Even effort across fronts is detected as a failure state, and concentration is refused when a front left behind cannot hold. |
| 346 | Local superiority > total-map unit count | ✅ | ✅ | `ConcentrationPolicy` | `ConcentrationPolicyTest` | **Closed.** Local superiority decides battles and total army size does not. Even effort across fronts is detected as a failure state, and concentration is refused when a front left behind cannot hold. |
| 347 | AI can mass forces against vulnerable area | ✅ | ✅ | `ConcentrationPolicy` | `ConcentrationPolicyTest` | **Closed.** Local superiority decides battles and total army size does not. Even effort across fronts is detected as a failure state, and concentration is refused when a front left behind cannot hold. |
| 348 | Other fronts retain defensive capability | ✅ | ✅ | `ConcentrationPolicy` | `ConcentrationPolicyTest` | **Closed.** Local superiority decides battles and total army size does not. Even effort across fronts is detected as a failure state, and concentration is refused when a front left behind cannot hold. |
| 349 | Concentration creates counterattack exposure | ✅ | ✅ | `ConcentrationPolicy` | `ConcentrationPolicyTest` | **Closed.** Local superiority decides battles and total army size does not. Even effort across fronts is detected as a failure state, and concentration is refused when a front left behind cannot hold. |
| 350 | Breakthrough followed by exploitation forces | ✅ | ✅ | `WaveComposition` (used by `StrategicBrainBotModule.ComposeWave`) | `CombinedArmsTest`, `MissionScenarioTest` | **Closed.** The combined-arms property is now named and asserted rather than being something a wave happened to have. Artillery needs a *ground* screen (aircraft do not screen it), AA escorts need something to escort, and a mass air attack is a concentration rather than aircraft trickling in. |
...
| 407 | Expansion risk evaluated | ✅ | ✅ | `ExpansionPolicy` | `ExpansionPolicyTest`, `AcceptanceOutcomeTest.CampaignFollowsAnIdentifiableSequence` | **Closed.** Expansion keyed off posture and local threat rather than a timer, garrisoned in proportion to risk from the moment the site is planned. |
| 408 | Expansion timing changes with posture | ✅ | ✅ | `ExpansionPolicy` | `ExpansionPolicyTest`, `AcceptanceOutcomeTest.CampaignFollowsAnIdentifiableSequence` | **Closed.** Expansion keyed off posture and local threat rather than a timer, garrisoned in proportion to risk from the moment the site is planned. |
| 409 | Ally assigned to expansion specialization | ✅ | ✅ | `ExpansionPolicy` | `ExpansionPolicyTest`, `AcceptanceOutcomeTest.CampaignFollowsAnIdentifiableSequence` | **Closed.** Expansion keyed off posture and local threat rather than a timer, garrisoned in proportion to risk from the moment the site is planned. |
| 410 | New expansions receive defensive planning | ✅ | ✅ | `ExpansionPolicy` | `ExpansionPolicyTest`, `AcceptanceOutcomeTest.CampaignFollowsAnIdentifiableSequence` | **Closed.** Expansion keyed off posture and local threat rather than a timer, garrisoned in proportion to risk from the moment the site is planned. |
| 411 | Economic vulnerability influences defense | ✅ | ✅ | `ExpansionPolicy` | `ExpansionPolicyTest`, `AcceptanceOutcomeTest.CampaignFollowsAnIdentifiableSequence` | **Closed.** Expansion keyed off posture and local threat rather than a timer, garrisoned in proportion to risk from the moment the site is planned. |
| 412 | Enemy economic weakness triggers raiding | ✅ | ✅ | `ExpansionPolicy` | `ExpansionPolicyTest`, `AcceptanceOutcomeTest.CampaignFollowsAnIdentifiableSequence` | **Closed.** Expansion keyed off posture and local threat rather than a timer, garrisoned in proportion to risk from the moment the site is planned. |
...
| # | Requirement | Impl | Test | Code | Tests | Notes |
|---|---|:--:|:--:|---|---|---|
| 500 | System prompt: coalition victory = primary objective | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 501 | LLM distinguishes known/observed/inferred/suspected/unknown | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed, and a real defect fixed getting here.** `team_summary` dropped the honesty-ladder breakdown the engine transmits, so the commander was asked to distinguish information it was never shown - unsatisfiable regardless of model. Now rendered, and the model leaves a stale position alone. |
| 502 | LLM instructed not to fabricate engine state | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 503 | LLM checks available forces before assigning | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 504 | LLM checks existing commitments before creating conflicts | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 505 | LLM identifies explicit strategic posture | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 506 | LLM identifies explicit main effort | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 507 | Major operations have explicit objectives | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 508 | Major operations have launch conditions | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 509 | Major operations have abort conditions | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 510 | Major operations have contingencies | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 511 | Major operations consider reserves | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 512 | Major operations consider enemy responses | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Anti-air production is requested against observed enemy air, with `ProductionContract` still contracting counters deterministically as a backstop. |
| 513 | Major operations consider recon requirements | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 514 | Major operations consider deception opportunities | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 515 | Major operations consider combined arms | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** See 512. |
| 516 | Major operations consider withdrawal/extraction | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 517 | LLM adapts when assumptions become invalid | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** The commander re-plans away from stale intel and adapts posture when outnumbered; both probes pass live across two runs. |
| 518 | LLM doesn't constantly rewrite valid plans | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
| 519 | LLM accepts strategically justified losses | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed, and a real defect fixed getting here.** The sanitizer replaced a deliberate no-attack target with the enemy centroid, converting 'defend, outnumbered seven to one' into an attack order. The substitution now applies only to plans that intend to attack. |
| 520 | LLM avoids strategically pointless losses | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** See 519; the model declines the hopeless fight and the sanitizer no longer overrides it. |
| 521 | LLM exploits major enemy mistakes rapidly | ✅ | ✅ | `ai/model_server.py` `SYSTEM_PROMPT`; engine backstops in `CommandValidator`/`CoalitionOrderArbiter` | `ai/commander_eval.py` (live Qwen3.5 4B), `ai/baselines/commander-behaviour.json` | **Closed.** Verified behaviourally against a live model rather than by asserting the instruction appears in the prompt. 6 of 8 probe groups pass with Qwen3.5 4B. |
...
| 548 | Controllers can report inability | ✅ | ✅ | `TacticalController.Unable`, `CoalitionCommandCenterBotModule.MayReplan` | `DoctrineMissionTest.ReplanRequestsAreDebounced` | **Closed.** The replan debounce is extracted as pure `MayReplan` and tested. Worth recording: `ExecuteTacticalForce` pre-checks each domain before calling `Attack`, so the controllers' `Unable` guard is a defensive check against double-claiming that does not fire in normal play (confirmed over a 9000-tick match). A test asserting it appears would have passed only by accident. |
| 549 | Controllers can request replanning | ✅ | ✅ | `TacticalController` base + `Ground`/`Air`/`Naval`/`Transport`/`SpecialOps` controllers; `TacticalFormation`, `TacticalEngagement` | `DoctrineMissionTest.ReplanRequestsAreDebounced` | **Closed.** Same debounce path: `Unable` → `RequestStrategicReplan` → `RequestReplan`, rate-limited to one replan per blackboard interval. |
...
| 571 | Support-power behavior covered by tests per RA power | ✅ | ✅ | `SupportPowerPolicy` (Redeployment/Protection), `mods/ra/rules/ai.yaml` | `SupportPowerTest.EveryRaPowerIsSupported`, `ForceMultiplierRequiresEscort`, `RegressionTest` | **Closed.** Chronosphere, Advanced Chronoshift and Iron Curtain now carry `Redeployment` and `Protection` roles and are wired into `ai.yaml`. Force multipliers invert the friendly-fire rule: they require a committed friendly force at the target rather than an empty cell. A test asserts every RA support power has a role, so adding one without a policy entry fails. |
...
| 604 | Economic damage caused recorded | ✅ | ✅ | `CoalitionMatchMetrics.EnemyEconomicDamage`, `IsEconomicAsset`/`CostOf` | `RegressionTest.EconomicDamageCountsValueNotBuildings`, `RebuildingDoesNotEraseDamage` | **Closed.** Economic damage is measured in credits across refineries, harvesters and silos; refinery counts are still reported alongside. Enemy value stays fair-fog limited to what the coalition can account for. |
| 605 | Economic damage suffered recorded | ✅ | ✅ | `CoalitionMatchMetrics.FriendlyEconomicDamage` | `RegressionTest` | **Closed.** Same value-based measure for damage suffered; a peak that only ratchets downward means an enemy rebuild cannot erase damage already recorded. |
...
| 622 | Opponent-model prediction accuracy recorded | ✅ | ✅ | `OpponentPredictionLog`, wired into `UpdateOpponentModel` | `OpponentPredictionTest` (9 cases) | **Closed.** `OpponentPredictionLog` scores the opponent profile's own forecasts (playstyle, tech direction, attack lane) against later observation - distinct from the combat estimator. Unresolved predictions never score as correct, repeated forecasts are not double-counted, resolution is final, and calibration reports whether the model is more confident when right (what makes `ShouldExploit` safe). |
...
| 643 | Performance vs standard bots measured | ✅ | ✅ | `ai/baselines/fixed-seed-matrix.json` | `StrategicBaselineTest.EveryOpponentIsMeasured` | **Closed.** Measured against all four scripted bots over three seeds each. |
| 644 | Performance vs multiple allied standard bots measured | ✅ | ✅ | `ai/baselines/fixed-seed-matrix.json` | `StrategicBaselineTest` | **Closed.** The scripted Normal baseline is measured over the same seeds for comparison. |
| 645 | Performance vs experienced human players evaluated | ✅ | 🟡 | `AnalyzeReplayCommand` | `ReplayAnalysisTest`; no recorded human game exists yet | **Capability closed.** `--analyze-replay` reads any OpenRA `.orarep`, including a game played against a human, and reports an explicit human-vs-AI verdict. The evaluation itself still needs a human to play a recorded game; the tooling no longer blocks it. |
...
| # | Requirement | Impl | Test | Code | Tests | Notes |
|---|---|:--:|:--:|---|---|---|
| 663 | Basic coordinated ground attack scenario | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 664 | Multi-player coalition attack | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 665 | Ground + artillery attack | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 666 | Ground + air attack | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 667 | Ground + naval support | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 668 | Ground + air + naval combined operation | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 669 | Feint followed by main assault | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 670 | Fake retreat into ambush | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 671 | Harvester raid | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 672 | Expansion denial | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 673 | Emergency base defense | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 674 | Reserve reinforcement | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 675 | Immediate counterattack after defense | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 676 | Air transport insertion | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 677 | Naval transport insertion | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 678 | Tanya operation | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 679 | Spy operation | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 680 | Engineer capture operation | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 681 | Transport rerouting after new threat | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 682 | Transport abort | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 683 | Special-unit extraction | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
| 684 | Simultaneous multi-front pressure | ✅ | ✅ | `MissionManager`, `CoalitionOrderArbiter`, `TransportStateMachine`, the doctrine contracts | `MissionScenarioTest` via `ScenarioHarness` | **Closed.** Driven end-to-end through the shipped subsystems with the situation set up deliberately, rather than running a match and hoping the scenario occurred. |
...
| 689 | LLM failure during major operation | ✅ | ✅ | `MissionType`/`MissionPhase` machinery + `TacticalControllers` | `HeadlessSkirmishTest.LlmDropoutMidBattleKeepsMissionsRunning` | **Closed.** `LlmDropoutMidBattleKeepsMissionsRunning` runs with the external brain enabled (exercising the request path and its timeout), then removes it outright and asserts the coalition keeps selecting posture and managing missions rather than merely surviving. |
...
| # | Requirement | Impl | Test | Code | Tests | Notes |
|---|---|:--:|:--:|---|---|---|
| 691 | AI tested on small maps | ✅ | ✅ | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.RunsOnSmallAndLargeMaps` | **Closed.** `RunsOnSmallAndLargeMaps` exercises the smallest playable conquest map. The blocker was assumed to be `Platform.OverrideEngineDir` being once-per-process, but that constrains the engine directory, not the map cache. |
| 692 | AI tested on large maps | ✅ | ✅ | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.RunsOnSmallAndLargeMaps` | **Closed.** Same test covers the largest playable conquest map, asserting completion, survivors and tick budget. |
...
| 694 | AI tested with many enemy players | ✅ | ✅ | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.ManyPlayersComplete` | **Closed.** A full eight-bot lobby completes inside the tick budget. |
| 695 | AI tested with hundreds of units | ✅ | ✅ | `HeadlessSkirmish.Result.PeakActorCount` | `HeadlessSkirmishTest.ReachesLargeUnitCount` | **Closed.** `ReachesLargeUnitCount` asserts peak simultaneous actors above 100 - measured 524 - so the previous `ActorCount > 0` assertion can no longer pass on a match that never grew. |
| 696 | AI tested with heavy air activity | ✅ | ✅ | `StrategicBrainBotModule.UpdateBuildPlan` arm-production telemetry, `ProductionBalance` | `HeadlessSkirmishTest.AirAndNavalArmsAreRaised`, `ProductionBalanceTest` | **Closed, after correcting an earlier misreading.** Aircraft are produced (heli queued 5x over a 20,000-tick match). The earlier "air 0" reading came from coordinated-force gate lines, which stop logging once the gate opens and therefore only ever showed the pre-production build-up. Production telemetry now distinguishes "arm never raised" from "arm never buildable". |
| 697 | AI tested with heavy naval activity | ✅ | ✅ | same arm-production telemetry | `HeadlessSkirmishTest.AirAndNavalArmsAreRaised` | **Closed.** The naval arm is reported by the same path; the test records an idle queue that can build nothing the plan wants separately from an arm that was never raised. |
| 698 | AI tested with many simultaneous missions | ✅ | ✅ | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.ReachesLargeUnitCount`, `SubsystemPerformanceTest.MissionManagementIsBounded` | **Closed.** Concurrent mission count is bounded and asserted alongside peak actor scale. |
| 699 | AI tested with frequent world-state changes | ✅ | ✅ | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.FrequentWorldStateChangesAreDebounced` | **Closed.** Event-driven reviews stay below the blackboard interval under rapid change. |
| 700 | AI does not cause unacceptable frame/tick degradation | ✅ | ✅ | `HeadlessSkirmish.Result.MeanTickMilliseconds` | `HeadlessSkirmishTest.TickCostWithinBudget` | **Closed.** `HeadlessSkirmish` records total/mean/slowest tick time; `TickCostWithinBudget` asserts the mean stays under the 40 ms real-time budget and no tick exceeds 500 ms. Measured: 1.17 ms mean, ~70 ms slowest. |
...
| 702 | Threat-map updates remain performant | ✅ | ✅ | threat aggregation over `CoalitionRegion.Threats` | `SubsystemPerformanceTest.ThreatAggregationIsBounded` | **Closed.** Bounded on a 256-region lattice, four times a large shipping map. |
| 703 | Route planning remains performant | ✅ | ✅ | `CoalitionRoutePlanner.FindRoute` | `SubsystemPerformanceTest.RoutePlanningIsBounded` | **Closed.** 200 routes over 256 regions across all four cost profiles, inside budget. |
| 704 | Mission-management complexity remains bounded | ✅ | ✅ | `MissionManager` | `SubsystemPerformanceTest.MissionManagementIsBounded` | **Closed.** Concluded missions are removed while their outcomes are still accounted, so the active list cannot grow for a whole match. |
| 705 | Memory leaks/actor-reference leaks tested | ✅ | ✅ | `CoalitionOrderArbiter`, `OpponentPredictionLog`, `EngagementOutcomeLog` | `SubsystemPerformanceTest.NoDanglingForceCommitments`, `OutcomeLogsStayProportional`, `HeadlessSkirmishTest.RepeatedMatchesReleaseResources` | **Closed.** Commitments are released with their mission, and the outcome logs grow with events rather than with tick count. |

### §Replay & Regression Testing  <span>(706–714)</span>

| # | Requirement | Impl | Test | Code | Tests | Notes |
|---|---|:--:|:--:|---|---|---|
| 706 | Deterministic/reproducible test seeds available | ✅ | ✅ | fixed-seed determinism in `HeadlessSkirmish`; timestamped `ai-telemetry.log` | `HeadlessSkirmishTest.DeterministicSameSeed`, `DifferentSeedsDiverge` | **Closed.** Reproducibility is only meaningful alongside divergence: seeds 4242 and 9999 produce identical actor counts at 1500 ticks, so the divergence check runs longer and compares the whole event fingerprint. |
| 707 | Known battle scenarios replayable automatically | ✅ | ✅ | `AnalyzeReplayCommand`, `HeadlessSkirmish` determinism | `ReplayAnalysisTest`, `HeadlessSkirmishTest.DeterministicSameSeed` | **Closed.** Fixed-seed headless re-runs plus real `.orarep` ingestion via `--analyze-replay`. |
| 708 | AI decisions inspectable alongside replay timestamps | ✅ | ✅ | `AnalyzeReplayCommand.AlignDecisions` | `ReplayAnalysisTest` (6 cases) | **Closed.** `AlignDecisions` places tick-stamped strategic decisions on the replay's own timeline. Decisions past the replay's end are excluded, because the telemetry log is appended across runs and attributing a later match's decisions here would be a false reading. |
| 709 | Previously discovered strategic bugs have regression tests | ✅ | ✅ | fixed-seed determinism in `HeadlessSkirmish`; timestamped `ai-telemetry.log` | `RegressionTest` | **Closed.** `RegressionTest` pins defects actually found and fixed in this work, each of which once failed. |
| 710 | Previously discovered transport bugs have regression tests | ✅ | ✅ | fixed-seed determinism in `HeadlessSkirmish`; timestamped `ai-telemetry.log` | `RegressionTest.NoMissionTypeBecomesUnreachable` | **Closed.** Transport/mission reachability is pinned by `NoMissionTypeBecomesUnreachable`, which catches the real defect class: a type added without a validator entry is silently unrequestable. |
...
| 712 | Previously discovered fog-of-war leaks have regression tests | ✅ | ✅ | fixed-seed determinism in `HeadlessSkirmish`; timestamped `ai-telemetry.log` | `RegressionTest`, `ExternalBrainSnapshotTest`, `IntelTrackerTest` | **Closed.** Fog regressions pinned by `InferredTargetsAreNotObservations` alongside the existing `ExternalBrainSnapshotTest`/`IntelTrackerTest`. |
| 713 | Combat estimation benchmarked against historical scenarios | ✅ | ✅ | `EngagementOutcomeLog` | `EstimatorBenchmarkTest.CorpusBrierScoreDoesNotRegress` | **Closed.** The corpus is scored with the same Brier machinery live engagements use, against a recorded baseline, so a change is measured against history rather than re-derived from itself. |
| 714 | Strategic behavior compared against baseline win rates | ✅ | ✅ | `ai/baselines/fixed-seed-matrix.json` | `StrategicBaselineTest` | **Closed.** The 12-match measurement is recorded and guarded: every opponent present, totals consistent, no seed dropped, and the 804 comparison written to fail the day the coalition overtakes the baseline. |
...
| 716 | Multiple maps in batch evaluation | ✅ | ✅ | `ai/selfplay.py --maps` | `ai/test_selfplay.py`, `HeadlessSkirmishTest.RunsOnSmallAndLargeMaps` | **Closed.** |
| 717 | Multiple factions/configurations included | ✅ | ✅ | `HeadlessSkirmish.CommanderFaction`, `SimulateCommand`, `ai/selfplay.py` | exercised by the cross-map/opponent batches | **Closed.** `FACTION=` on `--simulate`, `--faction` on `selfplay.py`, `HeadlessSkirmish.CommanderFaction`. An unknown faction is a hard error rather than a silent fallback that would mislabel the batch. |
| 718 | Strategic parameters varied experimentally | ✅ | ✅ | `ai/selfplay.py --sweep-*` | `ai/test_selfplay.py` | **Closed.** Each axis is verified to actually patch the ai.yaml key or export the environment variable it claims, against the real config, restored afterwards. A flag that parses but patches nothing is not a tunable parameter. |
| 719 | Threat weights tunable | ✅ | ✅ | `ai/selfplay.py --sweep-*` | `ai/test_selfplay.py` | **Closed.** Each axis is verified to actually patch the ai.yaml key or export the environment variable it claims, against the real config, restored afterwards. A flag that parses but patches nothing is not a tunable parameter. |
| 720 | Retreat thresholds tunable | ✅ | ✅ | `ai/selfplay.py --sweep-*` | `ai/test_selfplay.py` | **Closed.** Each axis is verified to actually patch the ai.yaml key or export the environment variable it claims, against the real config, restored afterwards. A flag that parses but patches nothing is not a tunable parameter. |
| 721 | Reserve percentages tunable | ✅ | ✅ | `ai/selfplay.py --sweep-*` | `ai/test_selfplay.py` | **Closed.** Each axis is verified to actually patch the ai.yaml key or export the environment variable it claims, against the real config, restored afterwards. A flag that parses but patches nothing is not a tunable parameter. |
| 722 | Production capability weights tunable | ✅ | ✅ | `ai/selfplay.py --sweep-*` | `ai/test_selfplay.py` | **Closed.** Each axis is verified to actually patch the ai.yaml key or export the environment variable it claims, against the real config, restored afterwards. A flag that parses but patches nothing is not a tunable parameter. |
| 723 | Target-scoring weights tunable | ✅ | ✅ | `ai/selfplay.py --sweep-*` | `ai/test_selfplay.py` | **Closed.** Each axis is verified to actually patch the ai.yaml key or export the environment variable it claims, against the real config, restored afterwards. A flag that parses but patches nothing is not a tunable parameter. |
| 724 | Feint commitment thresholds tunable | ✅ | ✅ | `ai/selfplay.py --sweep-*` | `ai/test_selfplay.py` | **Closed.** Each axis is verified to actually patch the ai.yaml key or export the environment variable it claims, against the real config, restored afterwards. A flag that parses but patches nothing is not a tunable parameter. |
| 725 | Special-ops risk thresholds tunable | ✅ | ✅ | `ai/selfplay.py --sweep-*` | `ai/test_selfplay.py` | **Closed.** Each axis is verified to actually patch the ai.yaml key or export the environment variable it claims, against the real config, restored afterwards. A flag that parses but patches nothing is not a tunable parameter. |
...
| 727 | Overfitting to one map/opponent checked | ✅ | ✅ | `ai/selfplay.py` (`--maps`, `--vs`, `--bot-type`, 8 `--sweep-*` axes, `--combat-accuracy`, `--details`) | `HeadlessSkirmishTest.RunsOnSmallAndLargeMaps` | **Closed for the C# suite.** The single-map monoculture that made overfitting likely is gone; `--maps` remains available for larger cross-map batches. |
...
| 729 | LLM plans scored for legality | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py`, `LlmEvalTest.PythonScorerSuitePasses` | **Closed.** `ai/test_llm_eval.py` exercises the shipped `score_legality`, and `LlmEvalTest` now runs that suite in a subprocess instead of re-implementing the algorithm in C#. |
| 730 | LLM plans scored for force availability | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py`, run from `ai/selfcheck.py` and `LlmEvalTest` | **Closed.** `score_force_availability()` is now covered by `ai/test_llm_eval.py` against the shipped module, including its boundary cases. |
| 731 | LLM plans scored for mission completeness | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py`, run from `ai/selfcheck.py` and `LlmEvalTest` | **Closed.** `score_mission_completeness()` is now covered by `ai/test_llm_eval.py` against the shipped module, including its boundary cases. |
| 732 | LLM plans scored for unnecessary risk | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py`, run from `ai/selfcheck.py` and `LlmEvalTest` | **Closed.** `score_unnecessary_risk()` is now covered by `ai/test_llm_eval.py` against the shipped module, including its boundary cases. |
| 733 | LLM plans compared with deterministic baseline | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py`, run from `ai/selfcheck.py` and `LlmEvalTest` | **Closed.** `score_baseline_comparison()` is now covered by `ai/test_llm_eval.py` against the shipped module, including its boundary cases. |
| 734 | LLM decisions checked for strategic oscillation | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py` | **Closed.** Same - `score_strategic_oscillation` is now tested against the shipped module. |
| 735 | LLM decisions checked for repeated impossible commands | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py`, run from `ai/selfcheck.py` and `LlmEvalTest` | **Closed.** `score_repeated_impossible()` is now covered by `ai/test_llm_eval.py` against the shipped module, including its boundary cases. |
| 736 | LLM decisions checked for misuse of uncertain intel | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py`, run from `ai/selfcheck.py` and `LlmEvalTest` | **Closed.** `score_uncertain_intelligence()` is now covered by `ai/test_llm_eval.py` against the shipped module, including its boundary cases. |
| 737 | LLM decisions checked for failing to maintain reserves | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py`, run from `ai/selfcheck.py` and `LlmEvalTest` | **Closed.** `score_reserves()` is now covered by `ai/test_llm_eval.py` against the shipped module, including its boundary cases. |
| 738 | LLM decisions checked for excessive idle forces | ✅ | ✅ | `ai/llm_eval.py` (11 `score_*` functions + `replay_same_state`) | `ai/test_llm_eval.py` | **Closed.** Same - `score_idle_forces` tested against the shipped module, including the 50% boundary. |
...
| # | Requirement | Impl | Test | Code | Tests | Notes |
|---|---|:--:|:--:|---|---|---|
| 789 | Unified Coalition Test (3+ allied AI as one command) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Three contingents under one mission id; a lower-priority mission attempting to grab one is refused with `REJECTED_CONFLICT`. |
| 790 | Combined Arms Test (ground+artillery+air+naval sync) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Air and naval shaping fires are planned to land on the same tick, with the slower force setting off earlier, both before the ground assault. |
| 791 | Deception Test (feint→enemy reaction→real attack) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Feint first, effect measured against a baseline, real attack then committed elsewhere. |
| 792 | Special Operations Test (Tanya/Spy transport+extract) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Judged worth committing, routed for stealth, and the cycle actually reaches Extract. |
| 793 | Human Attention Test (simultaneous coordinated threats) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Assault, raid, strike and special operation at once, with exactly one main effort. |
| 794 | Counter-Composition Test (enemy comp switch→production) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** A composition switch produces a different contract; producing the same units would not be a response. |
| 795 | Reserve Test (reserve remains, then reacts/exploits) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Reserve untouched while the attack runs, then answers an unexpected threat with the main effort intact. |
| 796 | Counterattack Test (enemy overcommits→AI counterattacks) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Fires into a depleted attacker, declines against an intact defence. |
...
| 799 | LLM Failure Test (LLM down mid-battle→missions continue) | ✅ | ✅ | end-to-end behaviour of the whole stack | `HeadlessSkirmishTest.LlmDropoutMidBattleKeepsMissionsRunning` | **Closed.** Same test: the simulation completes its full budget after the commander is gone, with posture selection and mission/production planning still running. |
...
| 801 | Adaptation Test (valid strategy→poor→cancel/modify) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** A stale plan is aborted and releases its force instead of staying committed to nothing. |
| 802 | Withdrawal Test (losing engagement→preserve forces) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Withdrawal preserves the force that withdrew, measured by retreat preservation rate. |
| 803 | Campaign Test (full match: recon→econ→pressure→ops→win) | ✅ | ✅ | end-to-end behaviour of the whole stack | `AcceptanceOutcomeTest` | **Closed.** Recon before economy before pressure before the main operation, asserted on creation order. |
| 804 | Brutal Fair-AI Test (extreme/supreme/fair/0% > standard) | 🟡 | ✅ | end-to-end behaviour of the whole stack | `HeadlessSkirmishTest` acceptance scenarios + `AcceptanceSuite` contracts | **Still not met, now measured properly.** Over a 12-match fixed-seed matrix (3 seeds x 4 opponents, Fair Fog, 0% bonus) Supreme scored **0W/5L/7D**; the standard Normal bot over the same seeds scored **4W/2L/3D** and beat Supreme 3-0 head-to-head at 0.39 exchange. Supreme's exchange ratio is better against Turtle (2.46) and Naval (3.27) but it converts none of those to a win - all six are time-limit draws. The prior claim of ~136% over baseline measured exchange on one matchup while the baseline was the side actually winning games. Root cause identified and a fix built (see 187/204 and `AdvanceOnInferredBase`), but the fix produced no wins in measurement and is therefore shipped disabled rather than enabled. |
