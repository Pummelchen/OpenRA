...
**Audited revision:** `24caf3753e` (post-remediation)
**Audit date:** 2026-08-23
**Method:** independent re-verification, followed by remediation of every finding. Build, full test
suite and Python self-check were executed; every subsystem was read at source level and every claim
traced to a named test.
...
|---|---:|---:|
| Complete and tested | 637 | **671** |
| Implemented but insufficiently tested | 153 | **132** |
| Partial | 14 | **1** |
| Missing | 0 | **0** |
| **Total** | **804** | **804** |
...
Thirteen of the fourteen partial requirements are closed, and all 22 previously untested
requirements now have tests. The single remaining partial is **804**, which is an empirical
outcome rather than a code gap — see the measurement below.

### The one requirement still open

**804 — "demonstrably much stronger than standard OpenRA bots."** Measured over a 12-match
fixed-seed matrix (3 seeds × 4 opponents, Fair Fog, 0% economic bonus):
...
Supreme wins no matches; the standard Normal bot wins four over the same seeds and beats Supreme
3–0 head-to-head. Supreme trades better against Turtle and Naval but converts none of it — all six
are time-limit draws. The earlier claim of "~136% above baseline" measured *exchange ratio* on one
matchup while the baseline was the side actually winning games.
...
| `dotnet test bin/OpenRA.Test.dll` | **863 passed, 2 skipped, 0 failed** |
| `.venv-ai/bin/python ai/selfcheck.py` | passed, including the new 11-scorer Python suite |
| Fixed-seed opponent matrix | 12 matches completed, reported above |
...
| 696 | AI tested with heavy air activity | ✅ | 🟡 | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.StressScale` (4 bots/3000 ticks), `RepeatedMatchesReleaseResources` | Covered indirectly by full-scale matches; no air-activity-specific scenario yet. |
| 697 | AI tested with heavy naval activity | ✅ | 🟡 | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.StressScale` (4 bots/3000 ticks), `RepeatedMatchesReleaseResources` | Covered indirectly by the naval-capable map in the cross-map test; no naval-specific load scenario yet. |
| 698 | AI tested with many simultaneous missions | ✅ | ✅ | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.ReachesLargeUnitCount` | **Closed.** Peak actor sampling plus the mission mix observed in long matches; tick cost is asserted to hold at full scale. |
| 699 | AI tested with frequent world-state changes | ✅ | 🟡 | `HeadlessSkirmish`, bounded scans/intervals, listener disposal | `HeadlessSkirmishTest.StressScale` (4 bots/3000 ticks), `RepeatedMatchesReleaseResources` | Long matches churn world state, but no scenario deliberately maximises churn. |
...
| 713 | Combat estimation benchmarked against historical scenarios | ✅ | 🟡 | fixed-seed determinism in `HeadlessSkirmish`; timestamped `ai-telemetry.log` | `HeadlessSkirmishTest.DeterministicSameSeed`, `ToolApiReleasesPortBetweenGames` | Per-engagement outcomes are now recorded (`EngagementOutcomeLog`), which is the corpus a historical benchmark needs, but no stored corpus is compared against across changes yet. |
